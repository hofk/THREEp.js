...
p = {
	...
	azimuthOpen,	// azimuthal connected or disconnected
	style,			// 'map', 'complete'
...
*/
	...
	g.azimuthOpen = 	p.azimuthOpen !== undefined ? 	p.azimuthOpe	: false;
...
function create() {
	...
	if ( g.bottomCircle < eqt ) {
		...
		vertexCount +=  g.bottomCircle === 0 ? 1 : 0;  // south pole
	...
	if ( g.topCircle > eqt ) {
		...
		vertexCount +=  g.topCircle === eqt * 2 ? 1 : 0; // north pole
	...
	if ( g.isBufferGeometry && g.indexed ) {
		...
		if ( g.bottomCircle < eqt ) {
			...
			verticesSum = g.bottomCircle === 0 ? 1 : 0;
		...
		if ( g.topCircle > eqt ) {
			...
			if ( g.topCircle === eqt * 2 ) { 
			
				a = g.vertexNorthOffset; // vertex 0 + north offset: north pole
				...
				for ( var j = 1; j <= wed; j ++ ) {
					...
					b ++;
					c = j !== wed	? c + 1 : 1 + g.vertexNorthOffset;
				...
				g.uvs[ g.vertexNorthOffset * 2 ] = 0.5;		
				g.uvs[ g.vertexNorthOffset * 2 + 1 ] = 0.5;
			...
			verticesSum = g.vertexNorthOffset + ( g.topCircle === eqt * 2 ? 1 : 0 );
...
function morphVertices( time ) {
	...
	var theta;
	var phi;
	var gap; // absolute half equator gap
	...
	if ( g.isBufferGeometry && g.indexed ) {	
		...
		if ( g.bottomCircle < eqt ) {
			...
			if ( g.bottomCircle === 0 ) {
				...
				// south pole (calculated from a very small circle)
				
				theta = - Math.PI / 2 + 0.01; // + .. because cos (theta) would be 0
				...
				for ( var j = 0; j < wed; j ++ ) { 
					...
					r = g.radius * g.rPhiTheta( nji, 0, t );
				...
				g.vertices[ 0 ]  = r * x / wed;		
				g.vertices[ 1 ]  = r * y - g.equatorGap / 2 * r;
				g.vertices[ 2 ]  = r * z / wed;
		...
		if ( g.topCircle > eqt  ) {
			...
			if ( g.topCircle === eqt * 2 ) { 
				...
				// north pole (calculated from a very small circle)
				...
				for ( var j = 0; j < wed; j ++ ) { 
					...
					r = g.radius * g.rPhiTheta( nji, 1, t );
				...
				g.vertices[ g.vertexNorthOffset * 3 ] = r * x / wed;;		
				g.vertices[ g.vertexNorthOffset * 3 + 1 ] = r * y + g.equatorGap / 2 * r; 
				g.vertices[ g.vertexNorthOffset * 3 + 2 ] =  r * z / wed;
											
				verticesSum = 1 + g.vertexNorthOffset; // only vertex 0 + g.vertexNorthOffset, north pole
				
			}
			
			for ( var i = eqt * 2 - g.topCircle; i <= Math.min( eqt, eqt * 2 - g.bottomCircle ); i ++ ) {
				
				setVertex( NORTH, g.equatorGap );
