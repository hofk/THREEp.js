...
'use strict';
		...
		// !!!!!  D E B U G  !!!!! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				...
				//outp.innerHTML = outp.innerHTML + " </br> </br> ";
	...
	if ( g.isBufferGeometry && !g.indexed ) {
		...
		function createPlane( sn, jMax, fOffset ) {
			...
			for ( var j = 0; j < jMax; j ++ ) {
				...
				if ( j > 0 ) {
					...
					//phi = 2 * Math.PI * g.scaleAzimuth( j / ( top * wed ) ) + phiOffset; // TEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		if ( g.bottom < eqt ) {
			...
			if ( g.top <= eqt && g.withTop ) {
				...
				vIdx = g.southTopVertex;  //  !!!  D E B U G  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		if ( g.top > eqt ) {
			...
			if ( g.bottom >= eqt && g.withBottom ) {
				...
				vIdx = g.northTopVertex; //  !!!  D E B U G  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
function morphVertices( time ) {
	...
	function xyzCalculation( sn ) {
		...
		if ( g.style === "relief" ) {
			...
			//x = r1 * ni * Math.cos( phi ) + pi2 * g.radius * g.moveX( nji, ni, t );
			...
			//z = -r1 * ni * Math.sin( phi ) +  sn * r0 + pi2 * g.radius * g.moveZ( nji, ni, t );
		...
		if (  g.style === "complete" && g.squeezeDefault ) {
			...
			// ry = r * ( sn === SOUTH ? g.stretchSouth( nji, nih, t ) : g.stretchNorth( nji, nih, t ) );
	
			x = r * Math.cos( theta ) * Math.cos( phi ) + g.radius * g.moveX( nji, ni, t );	
			
			// y = ry * Math.sin( theta ) + g.radius * ( sn * g.equatorGap( nji, t ) / 2 + g.moveY( nji, ni, t ) );
			y =  r * Math.sin( theta ); 
		...
		if (  g.style === "complete" && !g.squeezeDefault ) {
			...
			// r1y = r1 * ( sn === SOUTH ? g.stretchSouth( nji, nih, t ) : g.stretchNorth( nji, nih, t ) );
			
			x1 = r1 * Math.cos( theta ) * Math.cos( phi ) + g.radius * g.moveX( nji, ni, t );
			
			// y1 = r1y * Math.sin( theta ) + g.radius * ( sn * g.equatorGap( nji, t ) / 2 + g.moveY( nji, ni, t ) );
			y1 = r1 * Math.sin( theta );
			y1 *= ( sn === SOUTH ? g.stretchSouth( nji, nih, t ) : g.stretchNorth( nji, nih, t ) );
	...
	if ( g.isBufferGeometry && g.indexed ) {
		...
		if ( g.bottom < eqt ) {
			...
			if ( bottomS === 0 || g.withBottom ) {
				...
				vIdx = 0;
				setPoleVertex( SOUTH );		// uses values from setVertexPositions()
